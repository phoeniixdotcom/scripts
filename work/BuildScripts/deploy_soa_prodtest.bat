...
ECHO  use: deploy_soa_prodtest file.ear
ECHO -------------------------------------------------------------------------------
ECHO
...
:checkservers
FOR /F "tokens=*" %%G IN ('type c:\BuildScripts\soaprodtestservers.txt') DO (call :s_do_sums %%G)
GOTO :EOF
...
"c:\BuildScripts\psservice.exe" \\%HOST% -u csaweb\tca -p Monday01 stop JBAS50SVC
IF ERRORLEVEL 1 goto error7


ECHO [ Map network drive to deploy server ]
net use %REMOTEDRIVE% \\%HOST%\c$\jboss-5.1.0.GA\server\csa-with-activemq\deploy /USER:csaweb\tca Monday01
...
"c:\BuildScripts\psservice.exe" \\%HOST% -u csaweb\tca -p Monday01 start JBAS50SVC
...
SET CHECK=http://%HOST%/lookup-rs/checkWebService
ECHO %CHECK%
