...
ECHO -------------------------------------------------------------------------
ECHO - MXS                                                                   -
ECHO - CSA Build Deployer for IIS static sites                               -
ECHO - This script will deploy the latest war to the specified server.       -
ECHO - use: deploy_static website                                            -
ECHO - percent1 is the website to deploy to                                  -
ECHO -------------------------------------------------------------------------
ECHO.
...
SET LEVEL=0
SET REMOTEDRIVE=t:
SET REMOTESERVER=xian.csaweb.local
SET SOURCEFILES=.\
...
FOR %%A IN (%REMOTESERVER%) DO (
	...
	net use %REMOTEDRIVE% \\%%A\inetpub\wwwroot\%ENVIRONMENT% /USER:csaweb\administrator elafont4U
